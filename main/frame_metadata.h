...
typedef enum tnc_meta_type {
    ...
    META_TYPE_MON_TEXT,      // モニタテキスト出力（人間可読フォーマット）→ rx_ringbuf へ
    META_TYPE_RX_FRAME,      // 無線で受信した AX.25 フレーム（生データ + FCS）→ rx_ringbuf へ
    META_TYPE_TX_MON,        // 送信モニタ（送信したAX.25フレームの折り返し表示用）→ rx_ringbuf へ
} tnc_meta_type_t;
